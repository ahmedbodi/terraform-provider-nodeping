...
func TestDataSourceMetadata(t *testing.T) {
	...
	NewContactsDataSource().Metadata(context.Background(), fwdatasource.MetadataRequest{ProviderTypeName: "nodeping"}, resp)

	if want := "nodeping_contacts"; resp.TypeName != want {
...
func TestDataSourceSchemaIsValid(t *testing.T) {
	...
	NewContactsDataSource().Schema(ctx, fwdatasource.SchemaRequest{}, resp)
